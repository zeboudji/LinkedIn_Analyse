...
def generate_performance_graphs(excel_data):
    try:
        ...
        combined_df = pd.merge(engagement_df, abonnes_df, on='Date', how='left')
        combined_df['Posts per Day'] = combined_df['Date'].map(posts_per_day.set_index('Date')['Posts per Day']).fillna(0).astype(int)
        ...
        combined_df['Interactions'] = pd.to_numeric(combined_df['Interactions'], errors='coerce').fillna(0).astype(int)
        combined_df['Impressions'] = pd.to_numeric(combined_df['Impressions'], errors='coerce').fillna(0).astype(int)
        combined_df['Engagement Rate (%)'] = (combined_df['Interactions'] / combined_df['Impressions']) * 100
        combined_df['Engagement Rate (%)'] = combined_df['Engagement Rate (%)'].replace([pd.NA, pd.NaT, float('inf'), -float('inf')], 0)

        # Combiner avec les abonnés cumulés
        combined_df = pd.merge(combined_df, abonnes_df_clean[['Date', 'Cumulative Subscribers']], on='Date', how='left')

    ...
    try:
        ...
        **Recommandations :**
        ...
        for category in demographics_categories:
            ...
            # Vérifier s'il y a une colonne 'Valeur', sinon ajuster
            if 'Valeur' not in df_category.columns:
                df_category.rename(columns={'Value': 'Valeur'}, inplace=True)
...
if uploaded_file is not None:
    ...
    if results:
        ...
        with tab_advanced:
            ...
            # **1. Histogramme du Taux d'Engagement**
            st.subheader("Distribution du Taux d'Engagement")
            fig_engagement_hist = px.histogram(combined_df, x='Engagement Rate (%)',
                                              nbins=20,
                                              title='Distribution du Taux d\'Engagement',
                                              labels={'Engagement Rate (%)': 'Taux d\'Engagement (%)'},
                                              template='plotly_dark',
                                              color_discrete_sequence=[color_palette['Engagement Rate (%)']])
            st.plotly_chart(fig_engagement_hist, use_container_width=True)
            st.markdown("""
            **Interprétation :** Cet histogramme montre la distribution des taux d'engagement. 
            Il permet d'identifier les valeurs les plus fréquentes ainsi que les éventuelles anomalies ou valeurs extrêmes.
            """)
            # Bouton de téléchargement
            buf = fig_to_png(fig_engagement_hist)
            st.download_button(
                label="Télécharger cet histogramme en PNG",
                data=buf,
                file_name="engagement_histogram.png",
                mime="image/png",
            )

            # **2. Moyennes Mobiles du Taux d'Engagement**
            st.subheader("Moyenne Mobile du Taux d'Engagement (7 jours)")
            combined_df['Engagement Rate MA7'] = combined_df['Engagement Rate (%)'].rolling(window=7).mean()
            fig_engagement_ma = px.line(combined_df, x='Date', y='Engagement Rate MA7',
                                        title='Moyenne Mobile du Taux d\'Engagement sur 7 Jours',
                                        labels={'Engagement Rate MA7': 'Taux d\'Engagement MA7 (%)'},
                                        markers=False,
                                        template='plotly_dark',
                                        color_discrete_sequence=[color_palette['Engagement Rate (%)']])
            st.plotly_chart(fig_engagement_ma, use_container_width=True)
            st.markdown("""
            **Interprétation :** La moyenne mobile lisse les fluctuations quotidiennes du taux d'engagement, mettant en évidence les tendances à court terme.
            """)
            # Bouton de téléchargement
            buf = fig_to_png(fig_engagement_ma)
            st.download_button(
                label="Télécharger ce graphique en PNG",
                data=buf,
                file_name="engagement_moving_average.png",
                mime="image/png",
            )

            # **3. Top 10 des Jours avec le Meilleur Taux d'Engagement**
            st.subheader("Top 10 des Jours avec le Meilleur Taux d'Engagement")
            top10_engagement = combined_df.nlargest(10, 'Engagement Rate (%)')
            fig_top10 = px.bar(top10_engagement, x='Date', y='Engagement Rate (%)',
                               title='Top 10 des Jours avec le Meilleur Taux d\'Engagement',
                               labels={'Engagement Rate (%)': 'Taux d\'Engagement (%)', 'Date': 'Date'},
                               template='plotly_dark',
                               color='Engagement Rate (%)',
                               color_continuous_scale='Viridis')
            st.plotly_chart(fig_top10, use_container_width=True)
            st.markdown("""
            **Interprétation :** Ce graphique met en évidence les 10 jours où le taux d'engagement a été le plus élevé. 
            Analysez ces jours pour identifier les stratégies ou les contenus qui ont contribué à cet engagement exceptionnel.
            """)
            # Bouton de téléchargement
            buf = fig_to_png(fig_top10)
            st.download_button(
                label="Télécharger ce graphique en PNG",
                data=buf,
                file_name="top10_engagement_days.png",
                mime="image/png",
            )

            # **4. Agrégation Mensuelle des Principales Métriques**
            st.subheader("Agrégation Mensuelle des Principales Métriques")
            combined_df['Month'] = combined_df['Date'].dt.to_period('M').astype(str)
            monthly_metrics = combined_df.groupby('Month').agg({
                'Impressions': 'sum',
                'Interactions': 'sum',
            ...
            fig_monthly = make_subplots(
                rows=2, cols=2,
                subplot_titles=("Impressions Mensuelles", "Interactions Mensuelles", "Taux d'Engagement Moyen", "Nombre de Posts Mensuels"),
                specs=[[{"type": "bar"}, {"type": "bar"}],
                       [{"type": "line"}, {"type": "bar"}]]
            )

            fig_monthly.add_trace(
                go.Bar(x=monthly_metrics['Month'], y=monthly_metrics['Impressions'], name='Impressions',
                       marker_color=color_palette['Impressions']),
                row=1, col=1
            )

            fig_monthly.add_trace(
                go.Bar(x=monthly_metrics['Month'], y=monthly_metrics['Interactions'], name='Interactions',
                       marker_color=color_palette['Interactions']),
                row=1, col=2
            )

            fig_monthly.add_trace(
                go.Scatter(x=monthly_metrics['Month'], y=monthly_metrics['Engagement Rate (%)'], name='Taux d\'Engagement Moyen',
                           marker_color=color_palette['Engagement Rate (%)']),
                row=2, col=1
            )

            fig_monthly.add_trace(
                go.Bar(x=monthly_metrics['Month'], y=monthly_metrics['Posts per Day'], name='Nombre de Posts',
                       marker_color=color_palette['Posts per Day']),
                row=2, col=2
            )

            fig_monthly.update_layout(
                title_text="Agrégation Mensuelle des Principales Métriques",
                template='plotly_dark',
                height=800
            )
            ...
            st.markdown("""
            **Interprétation :** Cette série de graphiques présente les principales métriques agrégées par mois. 
            Elle permet d'identifier les tendances mensuelles et d'évaluer l'impact des stratégies mises en place sur ces périodes.
            """)
            # Bouton de téléchargement
            buf = fig_to_png(fig_monthly)
            st.download_button(
                label="Télécharger ce graphique en PNG",
                data=buf,
                file_name="monthly_aggregation.png",
                mime="image/png",
            )

            # **5. Comparaison entre la Croissance des Abonnés et le Taux d'Engagement**
            st.subheader("Comparaison entre la Croissance des Abonnés et le Taux d'Engagement")
            fig_growth_engagement = px.scatter(combined_df, x='Growth Rate', y='Engagement Rate (%)',
                                              title="Croissance des Abonnés vs Taux d'Engagement",
                                              labels={'Growth Rate': 'Taux de Croissance des Abonnés (%)', 'Engagement Rate (%)': 'Taux d\'Engagement (%)'},
                                              trendline="ols",
                                              template='plotly_dark',
                                              color='Growth Rate',
                                              color_continuous_scale='Viridis')
            st.plotly_chart(fig_growth_engagement, use_container_width=True)
            st.markdown("""
            **Interprétation :** Ce graphique examine la relation entre la croissance des abonnés et le taux d'engagement. 
            Une corrélation positive suggère que la croissance des abonnés est associée à une augmentation de l'engagement, tandis qu'une corrélation négative indique le contraire.
            """)
            # Bouton de téléchargement
            buf = fig_to_png(fig_growth_engagement)
            st.download_button(
                label="Télécharger ce graphique en PNG",
                data=buf,
                file_name="growth_vs_engagement.png",
                mime="image/png",
            )

            # **6. Histogramme du Nombre de Posts par Jour**
            st.subheader("Distribution du Nombre de Posts par Jour")
            fig_posts_hist = px.histogram(combined_df, x='Posts per Day',
                                         nbins=range(int(combined_df['Posts per Day'].min()), int(combined_df['Posts per Day'].max()) + 2),
                                         title='Distribution du Nombre de Posts par Jour',
                                         labels={'Posts per Day': 'Nombre de Posts'},
                                         template='plotly_dark',
                                         color_discrete_sequence=[color_palette['Posts per Day']])
            st.plotly_chart(fig_posts_hist, use_container_width=True)
            st.markdown("""
            **Interprétation :** Cet histogramme montre la fréquence des différents nombres de posts publiés chaque jour. 
            Il permet d'identifier les jours avec une haute ou une basse fréquence de publication.
            """)
            # Bouton de téléchargement
            buf = fig_to_png(fig_posts_hist)
            st.download_button(
                label="Télécharger cet histogramme en PNG",
                data=buf,
                file_name="posts_distribution.png",
                mime="image/png",
            )
